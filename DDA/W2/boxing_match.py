...
def find_closest(cat, ra, dec, max_dist):
  """ra,dec is radian"""
  n=len(cat)
  decs=cat[:,1]
  ...
  min=2*max_dist
  id=-1
  index = np.searchsorted(np.sort(decs), dec-max_dist, side='left')
  for i in range(index,n):
    if cat[sort_ind[i]][1]> dec+max_dist:
        break
    dist=angular_dist(cat[sort_ind[i]][0],cat[sort_ind[i]][1],ra,dec)
    if dist<=max_dist and dist< min:
      min=dist
      id=sort_ind[i]
  return (id,min)
def crossmatch(cat_1, cat_2, max_dist):
  ...
  for i in range(len(cat1)):
    id,min=find_closest(cat2,cat1[i][0],cat1[i][1],max_dist)
    if id>=0:
